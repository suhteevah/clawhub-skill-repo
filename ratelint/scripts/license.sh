#!/usr/bin/env bash
# RateLint -- License Validation Module
...
RATELINT_LICENSE_KEY="${RATELINT_LICENSE_KEY:-}"
...
# Retrieve the RateLint license key from environment variable or config file.
# Priority: ENV var > openclaw.json (python3 > node > jq fallbacks)
get_ratelint_key() {
  # 1. Check environment variable
  if [[ -n "$RATELINT_LICENSE_KEY" ]]; then
    echo "$RATELINT_LICENSE_KEY"
    return 0
  fi
...
try:
    with open('$OPENCLAW_CONFIG') as f:
        cfg = json.load(f)
    print(cfg.get('skills', {}).get('entries', {}).get('ratelint', {}).get('apiKey', ''))
except: pass
" 2>/dev/null)
...
try {
  ...
  console.log(cfg?.skills?.entries?.ratelint?.apiKey || '');
} catch(e) {}
" 2>/dev/null)

    # Try jq third
    elif command -v jq &>/dev/null; then
      key=$(jq -r '.skills.entries.ratelint.apiKey // ""' "$OPENCLAW_CONFIG" 2>/dev/null)
    fi
...
validate_license() {
  ...
  # Validate product matches ratelint
  if [[ "$product" != "ratelint" && -n "$product" ]]; then
    return 1
  fi
...
check_ratelint_license() {
  local required_tier="${1:-pro}"
  local required_level="${TIER_LEVELS[$required_tier]:-1}"

  # Step 1: Retrieve the license key
  local key
  if ! key=$(get_ratelint_key) || [[ -z "$key" ]]; then
    echo -e "${RED}${BOLD}--- RateLint License Required ---${NC}"
    echo ""
    echo -e "This feature requires a ${BOLD}${required_tier}${NC} license."
    echo ""
    echo -e "Get your license at: ${CYAN}${BOLD}https://ratelint.pages.dev/pricing${NC}"
    echo ""
    echo "Then configure it:"
    echo -e "  ${DIM}# In ~/.openclaw/openclaw.json:${NC}"
    echo -e "  ${CYAN}\"skills\": { \"entries\": { \"ratelint\": { \"apiKey\": \"YOUR_KEY\" } } }${NC}"
    echo ""
    echo -e "  ${DIM}# Or set the environment variable:${NC}"
    echo -e "  ${CYAN}export RATELINT_LICENSE_KEY=\"YOUR_KEY\"${NC}"
    return 1
  fi
  ...
  local payload
  if ! payload=$(decode_jwt_payload "$key"); then
    echo -e "${RED}[RateLint]${NC} Invalid license key format."
    echo -e "Get a valid license at: ${CYAN}https://ratelint.pages.dev/pricing${NC}"
    return 1
  fi
  ...
  if [[ -n "$product" && "$product" != "ratelint" ]]; then
    echo -e "${RED}[RateLint]${NC} License is for product '${BOLD}$product${NC}', not ratelint."
    echo -e "Get a RateLint license at: ${CYAN}https://ratelint.pages.dev/pricing${NC}"
    return 1
  fi
  ...
  if [[ "$actual_level" -lt "$required_level" ]]; then
    echo -e "${RED}[RateLint]${NC} This feature requires ${BOLD}$required_tier${NC} tier. You have: ${BOLD}$tier${NC}"
    echo -e "Upgrade at: ${CYAN}https://ratelint.pages.dev/upgrade${NC}"
    return 1
  fi
  ...
  if [[ -n "$expiry" ]]; then
    ...
    if [[ "$now" -gt "$expiry" ]]; then
      echo -e "${RED}[RateLint]${NC} License expired. Renew at: ${CYAN}https://ratelint.pages.dev/renew${NC}"
      return 1
    fi
  fi
...
show_ratelint_status() {
  local key
  if ! key=$(get_ratelint_key) || [[ -z "$key" ]]; then
    echo -e "License: ${YELLOW}Free tier${NC}"
    echo -e "  Patterns: 30 (RL, BF categories)"
    echo -e "  Upgrade at: ${CYAN}https://ratelint.pages.dev/pricing${NC}"
    return
  fi
  ...
  if [[ -n "$product" && "$product" != "ratelint" ]]; then
    ...
    echo -e "  This key is for $product, not RateLint."
    echo -e "  Get a RateLint license at: ${CYAN}https://ratelint.pages.dev/pricing${NC}"
    return
  fi
  ...
  case "$tier" in
    pro)
      echo -e "  Patterns: 60 (RL, BF, TH, BP categories)"
      echo -e "  Formats: text, json, html"
      echo -e "  Upgrade to Team for: QO, DD categories + full 90 patterns"
      ;;
    team)
