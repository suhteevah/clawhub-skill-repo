#!/usr/bin/env bash
# SchemaLint -- License Validation Module
...
SCHEMALINT_LICENSE_KEY="${SCHEMALINT_LICENSE_KEY:-}"
...
# Retrieve the SchemaLint license key from environment variable or config file.
# Priority: ENV var > openclaw.json (python3 > node > jq fallbacks)
get_schemalint_key() {
  # 1. Check environment variable
  if [[ -n "$SCHEMALINT_LICENSE_KEY" ]]; then
    echo "$SCHEMALINT_LICENSE_KEY"
    return 0
  fi
...
try:
    with open('$OPENCLAW_CONFIG') as f:
        cfg = json.load(f)
    print(cfg.get('skills', {}).get('entries', {}).get('schemalint', {}).get('apiKey', ''))
except: pass
" 2>/dev/null)
...
try {
  ...
  console.log(cfg?.skills?.entries?.schemalint?.apiKey || '');
} catch(e) {}
" 2>/dev/null)

    # Try jq third
    elif command -v jq &>/dev/null; then
      key=$(jq -r '.skills.entries.schemalint.apiKey // ""' "$OPENCLAW_CONFIG" 2>/dev/null)
    fi
...
validate_license() {
  ...
  # Validate product matches schemalint
  if [[ "$product" != "schemalint" && -n "$product" ]]; then
    return 1
  fi
...
check_schemalint_license() {
  local required_tier="${1:-pro}"
  local required_level="${TIER_LEVELS[$required_tier]:-1}"

  # Step 1: Retrieve the license key
  local key
  if ! key=$(get_schemalint_key) || [[ -z "$key" ]]; then
    echo -e "${RED}${BOLD}--- SchemaLint License Required ---${NC}"
    echo ""
    echo -e "This feature requires a ${BOLD}${required_tier}${NC} license."
    echo ""
    echo -e "Get your license at: ${CYAN}${BOLD}https://schemalint.pages.dev/pricing${NC}"
    echo ""
    echo "Then configure it:"
    echo -e "  ${DIM}# In ~/.openclaw/openclaw.json:${NC}"
    echo -e "  ${CYAN}\"skills\": { \"entries\": { \"schemalint\": { \"apiKey\": \"YOUR_KEY\" } } }${NC}"
    echo ""
    echo -e "  ${DIM}# Or set the environment variable:${NC}"
    echo -e "  ${CYAN}export SCHEMALINT_LICENSE_KEY=\"YOUR_KEY\"${NC}"
    return 1
  fi
  ...
  local payload
  if ! payload=$(decode_jwt_payload "$key"); then
    echo -e "${RED}[SchemaLint]${NC} Invalid license key format."
    echo -e "Get a valid license at: ${CYAN}https://schemalint.pages.dev/pricing${NC}"
    return 1
  fi
  ...
  if [[ -n "$product" && "$product" != "schemalint" ]]; then
    echo -e "${RED}[SchemaLint]${NC} License is for product '${BOLD}$product${NC}', not schemalint."
    echo -e "Get a SchemaLint license at: ${CYAN}https://schemalint.pages.dev/pricing${NC}"
    return 1
  fi
  ...
  if [[ "$actual_level" -lt "$required_level" ]]; then
    echo -e "${RED}[SchemaLint]${NC} This feature requires ${BOLD}$required_tier${NC} tier. You have: ${BOLD}$tier${NC}"
    echo -e "Upgrade at: ${CYAN}https://schemalint.pages.dev/upgrade${NC}"
    return 1
  fi
  ...
  if [[ -n "$expiry" ]]; then
    ...
    if [[ "$now" -gt "$expiry" ]]; then
      echo -e "${RED}[SchemaLint]${NC} License expired. Renew at: ${CYAN}https://schemalint.pages.dev/renew${NC}"
      return 1
    fi
  fi
...
show_schemalint_status() {
  ...
  if ! key=$(get_schemalint_key) || [[ -z "$key" ]]; then
    ...
    echo -e "  Patterns: 30 (IX, TY categories)"
    echo -e "  Upgrade at: ${CYAN}https://schemalint.pages.dev/pricing${NC}"
    return
  fi
  ...
  if [[ -n "$product" && "$product" != "schemalint" ]]; then
    ...
    echo -e "  This key is for $product, not SchemaLint."
    echo -e "  Get a SchemaLint license at: ${CYAN}https://schemalint.pages.dev/pricing${NC}"
    return
  fi
  ...
  case "$tier" in
    pro)
      echo -e "  Patterns: 60 (IX, TY, NM, RL categories)"
      echo -e "  Formats: text, json, html"
      echo -e "  Upgrade to Team for: QP, MG categories + full 90 patterns"
      ;;
    team)
