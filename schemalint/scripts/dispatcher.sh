#!/usr/bin/env bash
# SchemaLint -- Main CLI Dispatcher (Entry Point)
...
#   --category <cat>       Filter to specific category: IX, TY, NM, RL, QP, MG
#   --help, -h             Show help
...
log_info()    { echo -e "${BLUE}[SchemaLint]${NC} $*"; }
log_success() { echo -e "${GREEN}[SchemaLint]${NC} $*"; }
log_warn()    { echo -e "${YELLOW}[SchemaLint]${NC} $*"; }
log_error()   { echo -e "${RED}[SchemaLint]${NC} $*" >&2; }

# --- Version & Help -----------------------------------------------------------

show_version() {
  echo -e "${BOLD}SchemaLint${NC} v${VERSION}"
  echo "  Database schema anti-pattern analyzer"
  echo "  https://schemalint.pages.dev"
}
...
show_help() {
  ...
  echo -e "  --category <cat>       ${DIM}Filter: IX, TY, NM, RL, QP, MG${NC}"
  echo -e "  --help, -h             ${DIM}Show this help${NC}"
  echo -e "  --version, -v          ${DIM}Show version${NC}"
  echo ""
  echo -e "${BOLD}COMMANDS${NC}"
  echo -e "  scan                   ${DIM}Run database schema anti-pattern scan (default)${NC}"
  echo -e "  status                 ${DIM}Show license and configuration info${NC}"
  echo -e "  patterns               ${DIM}List all detection patterns${NC}"
  echo -e "  hooks install          ${DIM}Install lefthook git hooks${NC}"
  echo -e "  hooks uninstall        ${DIM}Remove lefthook git hooks${NC}"
  echo -e "  report                 ${DIM}Generate markdown report${NC}"
  echo ""
  echo -e "${BOLD}TIER SYSTEM${NC}"
  echo -e "  ${GREEN}FREE${NC}        30 patterns  IX, TY categories"
  echo -e "  ${CYAN}PRO${NC}  \$19/mo  60 patterns  IX, TY, NM, RL"
  echo -e "  ${MAGENTA}TEAM${NC} \$39/mo  90 patterns  IX, TY, NM, RL, QP, MG"
  echo -e "  ${BOLD}ENTERPRISE${NC}   90 patterns  All + priority support"
  echo ""
  echo -e "${BOLD}CHECK CATEGORIES${NC}"
  echo "  IX  Indexing Issues          -- missing FK indexes, composite order, over-indexing"
  echo "  TY  Type Safety              -- VARCHAR(255) overuse, FLOAT for money, INT booleans"
  echo "  NM  Naming Conventions       -- reserved words, plural/singular mix, abbreviations"
  echo "  RL  Relationships & Constraints -- missing FKs, no ON DELETE, circular refs"
  echo "  QP  Query Performance        -- SELECT *, N+1 setup, missing LIMIT, full scans"
  echo "  MG  Migration Safety         -- dropping columns, NOT NULL without default, lock-heavy"
  echo ""
  ...
  echo "  dispatcher.sh --path db/migrate/ --format json"
  echo "  dispatcher.sh --path . --category IX --verbose"
  echo "  dispatcher.sh --path . --format html > report.html"
  echo ""
  echo -e "Get a license at ${CYAN}https://schemalint.pages.dev/pricing${NC}"
}
...
resolve_tier() {
  ...
  if [[ -n "$tier_override" ]]; then
    ...
    if [[ $required_level -gt 0 ]]; then
      ...
      if [[ -n "$license_key_flag" ]]; then
        export SCHEMALINT_LICENSE_KEY="$license_key_flag"
      fi

      if ! check_schemalint_license "$tier_override"; then
        exit 1
      fi
    fi
    echo "$required_level"
  ...
  local detected_tier
  detected_tier=$(get_schemalint_tier)
...
do_list_patterns() {
  ...
  echo -e "${BOLD}--- SchemaLint Detection Patterns ---${NC}"
  echo ""
  echo -e "${BOLD}$(schemalint_pattern_count) patterns across 6 categories${NC}"
  echo ""

  schemalint_list_patterns "all"
}
...
do_hooks_install() {
  ...
  if [[ -f "$config" ]]; then
    if grep -q "schemalint" "$config" 2>/dev/null; then
...
# --- SchemaLint hooks ----------------------------------------
    schemalint-check:
      run: |
        SCHEMALINT_SKILL_DIR="${SCHEMALINT_SKILL_DIR:-$HOME/.openclaw/skills/schemalint}"
        if [[ -f "$SCHEMALINT_SKILL_DIR/scripts/analyzer.sh" ]]; then
          source "$SCHEMALINT_SKILL_DIR/scripts/patterns.sh"
          source "$SCHEMALINT_SKILL_DIR/scripts/analyzer.sh"
          hook_schemalint_check
        else
          echo "[SchemaLint] Skill not found at $SCHEMALINT_SKILL_DIR -- skipping scan"
        fi
      fail_text: |
        Schema issues detected in staged files!
        Run 'schemalint scan' to see details.
        Or 'git commit --no-verify' to skip (NOT recommended).
HOOKS
    echo -e "${GREEN}+${NC} Appended SchemaLint hooks to existing lefthook.yml"
  else
  ...
  echo ""
  log_success "Hooks installed! Staged files will be scanned for schema issues on every commit."
}
...
do_hooks_uninstall() {
  ...
  if [[ -f "$config" ]]; then
    if grep -q "schemalint" "$config" 2>/dev/null; then
      local tmp
      tmp=$(mktemp)
      sed '/# --- SchemaLint hooks/,/fail_text:.*$/d' "$config" > "$tmp"
      grep -v "schemalint" "$tmp" > "$config" 2>/dev/null || mv "$tmp" "$config"
      rm -f "$tmp"
      echo -e "${GREEN}+${NC} Removed SchemaLint hooks"
    else
      log_warn "No SchemaLint hooks found"
    fi
  else
...
do_scan() {
  ...
  # Run the scan
  do_schemalint_scan "$target" "$tier_level" "$format" "$verbose" "$category_filter"
}
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      hooks)
        ...
        case "$subcmd" in
          install)
            ...
            check_schemalint_license "pro" || exit 1
            do_hooks_install
            exit $?
            ;;
          uninstall)
      ...
      report)
        ...
        source "$SCRIPT_DIR/analyzer.sh"
        local tier_num
        tier_num=$(get_tier_level "$(get_schemalint_tier)")
        local categories
        categories=$(get_schemalint_categories_for_tier "$tier_num")
