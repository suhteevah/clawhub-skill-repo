#!/usr/bin/env bash
# SchemaLint -- Core Analysis Engine
...
SCHEMALINT_VERSION="${VERSION:-1.0.0}"
...
# Arguments:
#   $1 -- filepath to scan
#   $2 -- space-separated list of category codes (e.g., "IX TY NM RL")
...
scan_file() {
  ...
  for category in $categories; do
    ...
    local patterns_name
    patterns_name=$(get_schemalint_patterns_for_category "$category")
...
# Calculate a schema quality score based on findings.
...
print_category_breakdown_text() {
  ...
  for cat in IX TY NM RL QP MG; do
    local cat_count
...
print_score_text() {
  ...
  echo ""
  echo -e "${BOLD}Schema Quality Score: ${gc}${score}/100${NC} (Grade: ${gc}${BOLD}${grade}${NC})"
...
<head>
  ...
  <title>SchemaLint Report -- ${target}</title>
  <style>
...
<body>
  <h1>SchemaLint Report</h1>
  <p>Target: <code>${target}</code> | Date: ${date_str} | Version: ${SCHEMALINT_VERSION}</p>
  ...
  <table>
    ...
    <tr><td>Indexing Issues</td><td>IX</td><td>$(count_by_category "IX")</td></tr>
    <tr><td>Type Safety</td><td>TY</td><td>$(count_by_category "TY")</td></tr>
    <tr><td>Naming Conventions</td><td>NM</td><td>$(count_by_category "NM")</td></tr>
    <tr><td>Relationships & Constraints</td><td>RL</td><td>$(count_by_category "RL")</td></tr>
    <tr><td>Query Patterns</td><td>QP</td><td>$(count_by_category "QP")</td></tr>
    <tr><td>Migration Safety</td><td>MG</td><td>$(count_by_category "MG")</td></tr>
  </table>
  ...
  <div class="footer">
    Generated by <a href="https://schemalint.pages.dev">SchemaLint</a> v${SCHEMALINT_VERSION} on ${date_str}.
  </div>
</body>
</html>
HTML_EOF
...
generate_markdown_report() {
  ...
  if [[ $crit_count -gt 0 ]]; then
    recommendations="${recommendations}1. **CRITICAL:** ${crit_count} critical findings -- address immediately (data loss, SQL injection)${NEWLINE:-$'\n'}"
  fi
  if [[ $high_count -gt 0 ]]; then
    recommendations="${recommendations}2. **HIGH:** ${high_count} high-severity findings -- fix missing indexes, type mismatches, unsafe queries"$'\n'
  fi
  if [[ $med_count -gt 0 ]]; then
    recommendations="${recommendations}3. **MEDIUM:** ${med_count} medium findings -- review naming, constraints, and query patterns"$'\n'
  fi
  if [[ $low_count -gt 0 ]]; then
  ...
  if [[ ! -f "$template_file" ]]; then
    echo -e "${RED}[SchemaLint]${NC} Report template not found: $template_file" >&2
    return 1
  fi
  ...
  report="${report//\{\{VERSION\}\}/$SCHEMALINT_VERSION}"
...
#   category_filter -- optional category code to filter (e.g., "IX")
do_schemalint_scan() {
  local target="${1:-.}"
  ...
  local categories
  categories=$(get_schemalint_categories_for_tier "$tier_level")

  # Validate category filter if provided
  if [[ -n "$category_filter" ]]; then
    local filter_upper
    filter_upper=$(echo "$category_filter" | tr '[:lower:]' '[:upper:]')
    if ! is_valid_schemalint_category "$filter_upper"; then
      echo -e "${RED}[SchemaLint]${NC} Invalid category: ${BOLD}$category_filter${NC}" >&2
      echo -e "  Valid categories: IX, TY, NM, RL, QP, MG" >&2
      return 1
    fi
    ...
    if [[ "$cat_available" == false ]]; then
      echo -e "${RED}[SchemaLint]${NC} Category ${BOLD}$filter_upper${NC} is not available on your current tier." >&2
      echo -e "  Available categories for your tier: ${BOLD}$categories${NC}" >&2
      echo -e "  Upgrade at: ${CYAN}https://schemalint.pages.dev/pricing${NC}" >&2
      return 1
    fi
  fi
  ...
  if [[ $total_files -eq 0 ]]; then
    if [[ "$format" == "json" ]]; then
      ...
      echo -e "${GREEN}[SchemaLint]${NC} No scannable files found in ${BOLD}$target${NC}"
  ...
  if [[ "$format" == "text" ]]; then
    ...
    for cat in $categories; do
      local cnt
      cnt=$(schemalint_category_count "$cat")
      pattern_count=$((pattern_count + cnt))
    done

    echo -e "${BLUE}[SchemaLint]${NC} Scanning ${BOLD}$total_files${NC} files with ${BOLD}$pattern_count${NC} patterns..."
    if [[ -n "$category_filter" ]]; then
  ...
  case "$format" in
    ...
    text|*)
      if [[ ${#FINDINGS[@]} -eq 0 ]]; then
        echo -e "${GREEN}${BOLD}No schema or query issues detected!${NC}"
        echo ""
        echo -e "Schema Quality Score: ${GREEN}${BOLD}100/100${NC} (Grade: ${GREEN}${BOLD}A${NC})"
...
hook_schemalint_check() {
  ...
  if [[ ${#FINDINGS[@]} -eq 0 ]]; then
    echo -e "${GREEN}[SchemaLint]${NC} No schema or query issues in $file_count staged files."
    return 0
  fi
  ...
  echo -e "${RED}${BOLD}[SchemaLint] Schema/query issues detected in staged files!${NC}"
  echo ""
  ...
  if [[ "$has_blocking" == true ]]; then
    echo ""
    echo -e "${RED}Commit blocked.${NC} Fix schema/query issues before committing."
    echo -e "  Run: ${CYAN}schemalint scan${NC} for details"
